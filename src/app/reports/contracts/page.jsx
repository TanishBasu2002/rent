...
const RentAgreementsReport = () => {
    ...
    const handleDownloadCSV = () => {
        ...
        reportData.forEach(property => {
            ...
            property.rentAgreements.forEach(agreement => {
                const row = columnsRentAgreements.map(col => {
                    let value = col.english.split('.').reduce((acc, part) => acc && acc[part], agreement);
            ...
            const totals = property.rentAgreements.reduce((acc, agreement) => {
                ...
                return acc;
            }, { totalAmount: 0, paidAmount: 0, remainingAmount: 0, managementCommission: 0 });
        ...
        link.setAttribute('download', `تقرير_عقود_الإيجار_${dayjs().format('YYYY-MM-DD')}.csv`);
