          ...
          return {
            ...
            invoice: {
              ...
              rentAgreement: {
                ...
                status: needsAction
                  ? "NEEDSACTION"
                  : income.invoice.rentAgreement.status,
              },
            },
...
export async function getOwnersReport(page, limit, searchParams, params) {
  ...
  let propertiesWhere = {};
  propertiesWhere = await updateWhereClauseWithUserProperties(
    "id",
    propertiesWhere,
  );
  try {
...
export const getTaxPaymentsReport = async (page, limit, searchParams) => {
  const filters = searchParams.get("filters")
    ? JSON.parse(searchParams.get("filters"))
    : {};
...
export async function getRentAgreementsReports(page, limit, searchParams) {
  const filters = searchParams.get("filters")
    ? JSON.parse(searchParams.get("filters"))
    : {};
    ...
    properties.forEach((property) => {
      ...
      property.units.forEach((unit) => {
        unit.rentAgreements.forEach((agreement) => {
          ...
          agreement.managementCommission =
            agreement.totalAmount * (property.managementCommission / 100);
