...
const formatUAEPhoneNumber = (phoneNumber) => {
    ...
    if (cleaned.length < 9) {
        throw new Error('Phone number too short (UAE numbers must be at least 9 digits excluding country code)');
    }
 ...
 */
export const sendWhatsAppMessage = async (to, body, useTemplate = false, templateName = null) => {
    try {
        if (!client) {
            throw new Error('Twilio client not initialized due to missing credentials');
        }
        ...
        if (!isValidUAEWhatsAppNumber(formattedTo)) {
            throw new Error('Invalid UAE WhatsApp number. Must be a mobile number starting with +9715 and 9 digits long after country code.');
        }
        ...
        if (useTemplate && templateName) {
            ...
            const templateSid = process.env[`TWILIO_TEMPLATE_${templateName.toUpperCase()}`];
            if (!templateSid) {
                throw new Error(`Template "${templateName}" not found in environment variables`);
            }
        ...
        if (error.code === 63001 || (error.message && error.message.includes('outside the allowed window'))) {
            console.error('Error: Message failed - outside 24-hour messaging window. Use a template message instead.');
